...
import { dshHomePath } from './installer.ts'

/** User-editable `.env` template. Every setting is commented out by default. */
export const ENV_TEMPLATE = `# DeepSeek Harness 环境变量文件
 ...
 */
export async function ensureEnvTemplate(env: NodeJS.ProcessEnv = process.env): Promise<'created' | 'exists'> {
