...
export const CLOUDFLARED_DOWNLOAD_URL = `https://github.com/cloudflare/cloudflared/releases/download/${CLOUDFLARED_VERSION}/cloudflared-windows-amd64.exe`
 ...
 */
export async function downloadAndVerify(url: string, target: string, expectedSha256: string): Promise<void> {
  const response = await fetch(url, { redirect: 'follow' })
  if (!response.ok || response.body === null) {
    throw new Error(`cloudflare-tunnel: download failed with HTTP ${response.status} for ${url}`)
  }
  ...
  if (actual !== expectedSha256) {
    ...
    throw new Error(
      `cloudflare-tunnel: checksum mismatch for ${url}: expected ${expectedSha256}, got ${actual}`,
    )
  }
 ...
 */
export async function ensureManagedCloudflared(expectedSha256: string = CLOUDFLARED_SHA256): Promise<string> {
