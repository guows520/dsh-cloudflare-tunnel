...
async function apply(ctx, config) {
	...
	if (typeof config.hostname !== "string" || config.hostname.length === 0) {
		ctx.logger.warn("cloudflare-tunnel: hostname is not configured; tunnel not started. Set CLOUDFLARE_TUNNEL_HOSTNAME in your .dsh/.env file or in the plugin settings, then restart.");
